...
class TestCasing(unittest.TestCase):
    def test_uppercase(self):
        rule = UppercaseRule()
        self.assertTrue(rule.matches("THIS  IS A\n TEST, ALL UPPERCASE!", 0))
        self.assertFalse(rule.matches("THIS HAS A LOWERCASE LETTEr.", 0))

    def test_lowercase(self):
        rule = LowercaseRule()
        self.assertTrue(rule.matches("this  is a\n test, all lowercase.", 0))
        self.assertFalse(rule.matches("This has an uppercase letter.", 0))
